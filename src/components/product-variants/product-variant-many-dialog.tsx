...
interface ProductVariantManyDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  productVariant?: IProductVariant;
  onSave: (data: ProductVariantManyFormValues) => Promise<void>;
}

export function ProductVariantManyDialog({
  open,
  onOpenChange,
  productVariant,
  onSave,
}: ProductVariantManyDialogProps) {
  ...
  const form = useForm<ProductVariantManyFormValues>({
    resolver: zodResolver(productVariantManySchema),
    defaultValues: {
      price: 0,
      status: "ACTIVE",
      productId: "",
  ...
  const onSubmit = async (data: ProductVariantManyFormValues) => {
    await onSave(data);
    reset();
  };
